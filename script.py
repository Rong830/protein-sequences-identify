...
# 3. Check species number (too small?)(default)
...
# plotcon # infoalign # Cons # seqretsplit # patmatmotif # pepstats # pepwindowall # distmat
...
# Make sure the user have the packages installed
...
def edirect_check():
    ...
    if os.path.isdir(os.path.expanduser('~/edirect')):
        ...
        print_deco(
            'You do not have installed edirect in your home directory, I will install it for you.')
        subprocess.call(
            'sh -c "$(curl -fsSL ftp://ftp.ncbi.nlm.nih.gov/entrez/entrezdirect/install-edirect.sh)"', shell=True)
        print_deco('You have edirect in you home space, we can continue now.')


def output_path():
    # Ask the user where to put the output files, the default setting is the user home directory.
    ans = 'A'
    while ans.lower() != 'y' and ans.lower() != 'n':
        ans = input_deco(
            "Do you want to store all the output file in your current working directory? (y/n)")
    if ans.lower() == 'y':
        print_deco(
            "Sure! The output files will be stored in your current working directory.")
        return os. getcwd()
    if ans.lower() == 'n':
        ans = input_deco(
            "Sure! Please tell me the path name of where you want to store your output files.")
...
def pro_tax_check(pro, tax):
    num = int(subprocess.check_output(
        "esearch -db protein -query '{0}[prot]' -organism '{1}' | xtract -pattern ENTREZ_DIRECT -element Count".format(pro, tax), shell=True))
    if num == 0:
        return False
    return True
...
def ask_the_aim_pro():
    ...
    while test_special(pro) != True:
        pro = input_deco(
            "You in put included special character which is not allowed. \nPlease tell me the protein name you are interested in: ")
...
def pro_count(pro):
    count = int(subprocess.check_output(
        "esearch -db protein -query '{0}' | xtract -pattern ENTREZ_DIRECT -element Count".format(pro), shell=True))
    return count
...
def count_relative_protein(pro, tax):
    ...
    while count >= 1000:
        pro = input_deco(
            "WARNINGS! There are more than 1000 relative protein! Please check if you have enter the right protein name! \nPlease enter the right name of the protein family:\n")
        ask_the_aim_pro()
    while count == 0:
        pro = input_deco("WARNINGS! \nThere is no such "+str(pro) +
                         " protein in the database, Please start over!\nPlease enter the right name of the protein family:\n")
        ask_the_aim_pro()
...
def specific_download(pro, tax):
    ...
    for i in ['partial', 'low quality', 'hypothetical', 'predicted', 'isoform']:
        ans = input_deco('Do you want to include ' + str(i) +
                         ' in your target sequences? (y/n)')
        if ans.lower() == 'n':
            specific.append('NOT ' + str(i).upper() + ' ')
    print_deco('Your choice is: ' + str(specific) +
               ' Press any key to start downloading sequences.')
    file_check(str(pro) + '_' + str(tax) + ".fasta")
    subprocess.call("esearch -db protein -query "+str(pro)+"[prot] -organism "+str(tax)+''.join(
        specific)+" | efetch -format fasta > " + str(pro) + '_' + str(tax) + ".fasta", shell=True)
    # Download the data and count the number of sequences and species.
    file_check(str(pro) + "_" + str(tax) + ".txt")
    subprocess.call(
        'esearch -db protein -query "{0}[prot]" -organism "{1}" | efetch -format docsum | xtract -pattern DocumentSummary -element Organism > {0}_{1}.txt'.format(pro, tax), shell=True)
...
def clustalo_align(pro, tax):
    ...
    # With '--force' we don't need to check whether the output file is existed.
    subprocess.call('clustalo -i ' + str(pro) + '_' + str(tax) + '.fasta -o ' +
                    str(pro) + '_' + str(tax) + '_align.fasta -v --force', shell=True)


def hydropathy(pro, tax):
    print_deco('Generating Hydropathy plot for alignment')
    subprocess.call('pepwindowall ' + str(pro) + '_' + str(tax) +
                    '_align.fasta -graph svg -goutfile hydropathy', shell=True)
    print_deco(
        'The Hydropathy plot is generated, it has been stored in hydropathy')
...
def plotcon(filename):
    ...
    subprocess.call('plotcon -sequence ' + str(filename) +
                    ' -winsize 4 -graph svg', shell=True)
    # Ask the user if he/she wants to see the result.
    ans = input_deco("Do you want to see the graph of the conservation? (y/n)")
    if ans.lower() == 'y':
...
def clustalo_percent_change(pro, tax):
    # Ask the user if he/she want to calculate the percentage changes by using infoalign.
    ans = input_deco(
        "Do you want to calculate the percentage changes by using infoalign? (y/n) ")
    if ans.lower() == 'y':
        # download name part and percentage change part, and make a dictionary after sort.
        print_deco("I'm calculating now, just be patient.")
        file_check('output1.infoalign')
        file_check('output2.infoalign')
        subprocess.call('infoalign -sequence ' + str(pro) + '_' + str(tax) +
                        '_align.fasta -only -name -outfile output1.infoalign', shell=True)
        subprocess.call('infoalign -sequence ' + str(pro) + '_' + str(tax) +
                        '_align.fasta -only -change -outfile output2.infoalign', shell=True)
        ...
        ans = input_deco(
            "The percentage changes are sorted from low to high.\nDo you want to store them? (y/n) \n\n"+str(dic))
        if ans == 'y':
            # Store the sequences to the file called acc_bitscore_sorted.txt
...
def consensus_sequence(pro, tax):
    ...
    subprocess.call('cons -plurality 0.8 -sequence ' + str(pro) + '_' + str(tax) +
                    '_align.fasta -outseq ' + str(pro) + '_' + str(tax) + '_consensus.fasta', shell=True)
...
def create_blastdb(pro, tax):
    ...
    print_deco('I am creating a blast database from fasta files')
    file_check(str(pro) + "_" + str(tax) + "_database")
    subprocess.call("makeblastdb -in " + str(pro) + "_" + str(tax) +
                    ".fasta -dbtype prot -out " + str(pro) + "_" + str(tax) + "_database", shell=True)
...
def pepstats():
    ...
    subprocess.call(
        "seqretsplit -sequence ../acc_bitscore_sorted.fasta -sformat fasta -osformat fasta -auto", shell=True)
    # Uses EMBOSS pepstats to get protein statistics from the fasta files and save them to an output.
    file_name = []
    file_name = glob.glob("./*.fasta")
    for i in file_name:
        subprocess.call("pepstats -sequence " + str(i) +
                        " -outfile " + str(i) + ".pepstats", shell=True)
...
def PROSITE_db_check(prosite_db_directory='/localdisk/software/EMBOSS-6.6.0/share/EMBOSS/data/PROSITE/prosite.lines'):
    if os.path.exists(prosite_db_directory):
        print_deco(
            "You have PROSITE database in your home directory, we can continue to the next step.")
    else:
        print('Prosite file \"prosite.lines\" was not found. Attempting to create by downloading .dat & .doc files from EMB=EBI')
        home = os.path.expanduser('~')
        print_deco(
            "I'm checking if you have already download the PROSITE database in your home directory.")
        if os.path.exists(home+"/prosite/prosite.doc") and os.path.exists(home+"/prosite/prosite.dat"):
            print_deco(
                'You have all the database file we needed. We can continue to the next step.')
        else:
            print(
                "You don't have the files we needed.\nI'm downloading everything you need now.")
            if not os.path.exists(home+"/prosite"):
                os.mkdir(home+"/prosite")
                subprocess.call("wget http://ftp.ebi.ac.uk/pub/databases/prosite/prosite.doc -O " +
                                home+"/prosite/prosite.doc", shell=True)
                subprocess.call("wget http://ftp.ebi.ac.uk/pub/databases/prosite/prosite.dat -O " +
                                home+"/prosite/prosite.dat", shell=True)
                print_deco(
                    'You have all the database file we needed. We can continue to the next step.')
                print_deco(
                    "Uses prosextract to process prosite database before we start searching.")
...
def motifs_search():
    ...
    for i in file_name:
        ...
        subprocess.call("patmatmotifs -sequence " + str(i) +
                        " -auto Y -outfile " + str(file_motif), shell=True)
...
def summary_email():
    ...
    file.write('Files Analysed:' + str(len(motif_files)) +
               ' motifs found in: ' + str(len(motif_files)) + str("\n"))
    for k in motif_dict.keys():
        file.write('In file: ' + str(k) + ' We have found ' + str(
            motif_dict[k]['no_motifs']) + ' motifs, and the motif(s) is: ' + str(motif_dict[k]['motifs']) + '\n')
    file.close()
    ...
    if ans.lower() == 'y':
        ...
        subprocess.call('mail -s "$DATE" < summary.txt ' +
                        str(ans), shell=True)


def create_pd_from_blastp(file):
    headers = ['query_acc.', 'subject_acc.', '%_identity', 'alignment_length', 'mismatches',
               'gap_opens', 'query_start', 'query_end', 'subject_start', 'subject_end', 'e-value', 'bit_score']
...
def distmat():
    ...
    if ans.lower() == 'y':
        subprocess.call(
            'distmat -sequence ../acc_bitscore_sorted.fasta -outfile distmat.txt -protmethod 0', shell=True)
        print_deco(
            'The distance matrix is now generated and stored in a file called "distmat.txt".')


def PCA_plot(output_path, pro, tax):
    # Open the alignment output file
    file = open(str(output_path) + "/"+str(pro)+"_"+str(tax) +
                "/"+str(pro)+"_"+str(tax)+"_align.fasta", 'r')
    ...
    ctype = CategoricalDtype(['I', 'Y', 'G', 'Q', 'L', 'S', 'W', 'M',
                             'N', 'H', '-', 'V', 'D', 'E', 'F', 'A', 'R', 'K', 'C', 'P', 'T'])
    ...
    ans = input_deco(
        "Do you want to visualize the sequences variances in a 3D graph? (y/n)")
    # while ans.lower() != 'y' and ans.lower() != 'n':
...
def PCA_3D_plot(X):
    ...
    ax = Axes3D(fig, rect=[0, 0, .95, 1], elev=30,
                azim=300, auto_add_to_figure=False)
    fig.add_axes(ax)
    ax.scatter(X_3d[:, 0], X_3d[:, 1], X_3d[:, 2])
    plt.legend(loc='center left', bbox_to_anchor=[1.1, .5])
    plt.title('The variance of the protein sequences in 3D PCA')
    plt.savefig("3D_PCA.png", transparent=True)
    plt.show()
    print_deco('The graph is stored and named as "3D_PCA.png".')


def distmap_heatmap(output_path, pro, tax):
    ans = input_deco(
        "Do you want to generate a heatmap for distant matrix? (y/n)")
    if ans.lower() == 'y':
        subprocess.call(
            'clustalo -i '+str(output_path)+'/'+str(pro)+'_'+str(tax)+'/acc_bitscore_sorted.fasta --distmat-out=DISTMAT --full', shell=True)
        file = open("DISTMAT", 'r')
        dist = []
        lines = file.readlines()
        for l in lines:
        ...
        sns.clustermap(dist_arr.drop([0], axis=1).astype(
            float), cmap='rainbow', yticklabels=dist_arr[0])
        plt.savefig("Heatmap.png", transparent=True)
        plt.show()
        print_deco('The graph is stored and named as "Heatmap.png".')
    # Sorry no time to finish the contours plot. (T＿T)
    # I also want to try k-means, but I think that is exactly what clustalo has done.
    # ans = input_deco(
    #     "Do you want to visualize the sequences variances in a contours plot? (y/n)")
    # # while ans.lower() != 'y' and ans.lower() != 'n':
    # #     ans = input_deco("Do you want to visualize the sequences variances in a 3D graph? (y/n)")
    # if ans.lower() == 'y':
    #     Contours_plot(np.ndarray(dist_arr).reshape(1,np.shape(dist_arr)[0]*np.shape(dist_arr)[1]))
...
#     X = reX(X)
#     mean = X.mean(0)
#     cov = np.einsum('ijk,ikj->jk', X-mean, X-mean) / (X.shape[0]-1)
#     return cov, mean
# def prob(X):
#     cov, mean = fit(X)
#     X = reX(X)
#     a = (2*np.pi)**(-mean.shape[0]/2)*np.linalg.det(cov)**(-1/2)
#     b = np.exp((-1/2)*np.einsum('ijk,jl,ilk->ik',X-mean,np.linalg.inv(cov),X-mean))
#     return a*b
...
def Contours_plot(X):
    ...
    xx, xy = np.meshgrid(*np.linspace(0, 50, np.shape(X)
                         [0])[np.newaxis, ...].repeat(np.shape(X)[1], 0))
    # xx,xy = np.meshgrid(*np.linspace(0,50,np.shape(X)[0])[np.newaxis,...].repeat(np.shape(X)[1],0))
    # m = np.array([xx,xy]).transpose((1,2,0)).reshape(-1,2,1)
    # p = prob(m)
    cs = plt.contour(xx, xy, np.reshape(np.shape(X)[0], np.shape(X)[0]))
    ...
    plt.title('Contours Plot for all the sequences')
    plt.savefig("Contours Plot for all the sequences", transparent=True)
    plt.show()
...
edirect_check()
# Ask the user where to store the output files. [I start to feel output path is much more convenience! However it is too late.]
...
print_deco(
    "Thanks for your cooperation, now I made a directory for you named " + str(pro) + str(tax))
...
print_deco(
    "Sure! Now I will download the fasta files that related to what you searching for.")
subprocess.call(
    'esearch -db protein -query "{0}[prot]" -organism "{1}" | efetch -format fasta > {0}_{1}.fasta'.format(pro, tax), shell=True)
...
print_deco('Using blastp to search this file: ' + str(pro) +
           '_' + str(tax) + '_consensus.fasta in blastdb')
subprocess.call('blastp -db ' + str(pro) + '_' + str(tax) + '_database -query ' + str(pro) + '_' +
                str(tax) + '_consensus.fasta -outfmt 7 -out ' + str(pro) + '_' + str(tax) + '_blast.txt', shell=True)

# Create a dictionary that contains the acc and the bit score.
blastp_result = {}
line = []
line = [i for i in open(str(pro) + '_' + str(tax) +
                        '_blast.txt', 'r') if '#' not in i]
...
ans = input_deco(
    "I have sorted the related sequences by their bit score.\nHow many sequence you want to store? \nPlease enter an number.")
while True:
    ans = test_number(ans)
    if ans <= 0:
        ans = input_deco("The number is negative. \nPlease enter again.")
    else:
        break
...
PROSITE_db_check()
# Make a directory to store the separate sequences files
mkdir("motif")
os.chdir("motif")
pepstats()
...
print_deco("Now I'm extracting sequences from a fasta file")
subprocess.call('/localdisk/data/BPSM/Assignment2/pullseq -i '+str(output_path)+'/' + str(pro) + '_' + str(tax) + '/' + str(pro) + '_' + str(tax) +
                '_align.fasta -n '+str(output_path)+'/' + str(pro) + '_' + str(tax) + '/acc_bitscore_sorted.txt > '+str(output_path)+'/' + str(pro) + '_' + str(tax) + '/acc_bitscore_sorted.fasta -v', shell=True)
...
ans = input_deco(
    "Do you want to see the plot of blast result by plotcon? (y/n)")
if ans.lower() == 'y':
...
distmap_heatmap(output_path, pro, tax)
...
print_deco("This is the end of this program. \nHope your experience is good. \nThanks for using this program.\nHave a nice day~~BYE")
